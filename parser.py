...
def scrape_pinterest_fallback(url):
    """Scrape Pinterest pin page for video/image content."""
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36',
        'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8',
        'Accept-Language': 'zh-TW,zh-Hant;q=0.9,en;q=0.8',
    ...
    try:
        ...
        title_m = re.search(r'<meta\s+property="og:title"\s+content="([^"]*)"', html_text) or re.search(r'<title>([^<]*)</title>', html_text)
        title = html_lib.unescape(title_m.group(1)) if title_m else "Pinterest 貼圖"
        ...
        # Try to find video in JSON state blob
        state_m = re.search(r'__PWS_INITIAL_PROPS__\s*=\s*(\{.*?\});', html_text, re.DOTALL) or \
                  re.search(r'__PWS_DATA__\s*=\s*(\{.*?\});', html_text, re.DOTALL)

        video_url = None
        if state_m:
            try:
                state_text = state_m.group(1)
                # Look for video URL patterns in the JSON blob
                vid_m = re.search(r'"V_720P":\s*\{"url":"([^"]+)"', state_text) or \
                        re.search(r'"V_480P":\s*\{"url":"([^"]+)"', state_text) or \
                        re.search(r'"video_list":\s*\{[^}]*"url":"([^"]+\.m3u8[^"]*)"', state_text) or \
                        re.search(r'"url"\s*:\s*"(https://v1\.pinimg\.com/[^"]+\.mp4[^"]*)"', state_text)
                if vid_m:
                    video_url = html_lib.unescape(vid_m.group(1).replace('\\/', '/'))
            except Exception:
                pass

        # Also search entire HTML for Pinterest video URLs
        if not video_url:
            vid_urls = re.findall(r'https://v1\.pinimg\.com/[^\s"\']+\.mp4[^\s"\']*', html_text)
            if vid_urls:
                video_url = html_lib.unescape(vid_urls[0].replace('\\/', '/'))
        ...
        clean_images = []
        if thumbnail:
            # Upgrade Pinterest thumbnail to max resolution
            hq_thumb = re.sub(r'/\d+x/', '/originals/', thumbnail)
            clean_images.append(hq_thumb if hq_thumb != thumbnail else thumbnail)

        # Also check og:image and additional images in HTML
        img_urls = re.findall(r'https://i\.pinimg\.com/originals/[^\s"\']+\.jpg[^\s"\']*', html_text)
        for img in img_urls:
            ci = html_lib.unescape(img.replace('\\/', '/'))
        ...
        return {"success": False, "error": f"Pinterest 解析失敗: {str(e)}"}
...
def parse_url(target_url):
    ...
    if 'pinterest.com' in clean_target_url or 'pin.it' in clean_target_url:
        pin_res = scrape_pinterest_fallback(clean_target_url)
        ...
        with YoutubeDL(ydl_opts) as ydl:
            ...
            for f in raw_formats:
                ...
                if vcodec != 'none':
                    ...
                    # For YouTube: tag progressive (combined) formats as 'direct' so server can stream without merge
